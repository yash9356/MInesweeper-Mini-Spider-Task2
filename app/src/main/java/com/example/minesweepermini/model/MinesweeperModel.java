package com.example.minesweepermini.model;
...
public class MinesweeperModel {
    ...
    private short[][] cover = {
            {UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED,UNTOUCHED,UNTOUCHED,UNTOUCHED},
            {UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED,UNTOUCHED,UNTOUCHED,UNTOUCHED},
            {UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED,UNTOUCHED,UNTOUCHED,UNTOUCHED},
            {UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED,UNTOUCHED,UNTOUCHED,UNTOUCHED},
            {UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED, UNTOUCHED,UNTOUCHED,UNTOUCHED,UNTOUCHED},
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if(model[i][j] != MINE) {
                    ...
                    if (j < 7 && model [i][j+1] == MINE) {
                        mineCounter++;
                    }
                    if (i < 7 && j > 0 && model [i+1][j-1] == MINE) {
                        mineCounter++;
                    }
                    if (i < 7 && model [i+1][j] == MINE) {
                        mineCounter++;
                    }
                    if (i < 7 &&  j < 7 && model [i+1][j+1] == MINE) {
                        mineCounter++;
                    }
    ...
    public boolean checkAllTiles() {
        ...
        return minesFlagged == countMines() ;
    }
